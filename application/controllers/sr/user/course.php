...
class Course extends Frontend_Controller{
	...
	* 	Description: Allows user to select a course they would like to view
	...
	public function select($slug=NULL, $difficulty='all'){
		...
		//Get all the courses with the category_id
			//$this->data['course_data'] =$this->course_m->get_by_category($category->category_id);
			if($difficulty==='jednostavno'){
				$difficulty_trans='all';
			}elseif($difficulty==='napredno'){
				$difficulty_trans='hard';
			}elseif($difficulty==='umereno'){
				$difficulty_trans='medium';
			}elseif($difficulty==='lako'){
				$difficulty_trans='easy';
			}else{
				$difficulty_trans='all';
			}
		$diff=$this->difficulty_m->get_by_slug($difficulty_trans);

	 if($category !=NULL){
		if($difficulty=='sve' OR $diff[0]->difficulty_id == 1){
		  ...
		  //-----END---------LOADING DATA-------------------
			...
			//----------------------- SIDEBAR VIEW INFORMATION
				...
				foreach($all_lessons as $lesson){
					...
					$this->data["large_links"] .=
					'<div class="col-lg-10 challenges-list" style="margin:5px;">
